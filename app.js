      ...
      var expandedSetlists = {};     // { [slId]: true } – individuelle Karten-Inhalte
      var setlistSectionExpanded = true; // Section-Level: Karten sichtbar ja/nein
      var liedSuchModus = "";
      ...
      // SVG-Icon für Spickzettel – identisch mit dem Icon auf den Liedkarten.
      // Wird in Hero-Buttons und Subtab-Label genutzt.
      var SPICK_ICON = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-.15em" aria-hidden="true"><rect x="4" y="2" width="16" height="20" rx="2"/><path d="M9 8h6M9 12h6M9 16h4"/></svg>';

      // Tipps rotieren täglich (Index = Tag des Jahres mod Anzahl Tipps).
      // Neue Tipps können einfach ans Array angehängt werden.
      var DASH_TIPPS = [
        {
          eyebrow: "Umgang mit der Gruppe",
          titel: "Wenn die Gruppe unruhig wird",
          body: "Bekanntes Lied einwerfen – das schafft Sicherheit. Gemeinsam atmen. Körper aktivieren: klatschen, schnippen, stampfen. Stille zulassen – nicht sofort auffüllen."
        },
        {
          eyebrow: "Stimme & Aufwärmen",
          titel: "Summen auf 'mmh'",
          body: "Lippen locker, Resonanz spüren – 1 Minute reicht, um die Stimme zu wecken, ohne sie zu fordern. Ideal zu Beginn jeder Singstunde."
        },
        {
          eyebrow: "Liedauswahl",
          titel: "Bekanntes als Anker",
          body: "Bekanntes Lied als sicheren Anker setzen, neues Lied darin einbetten. Bei Demenz: Lieder aus der Jugendzeit (1940–1960) mit klarer Melodie und langsamem Tempo."
        },
        {
          eyebrow: "Schwierige Situationen",
          titel: "Jemand fängt an zu weinen",
          body: "Präsent bleiben, ruhig weitersingen. Raum lassen – Worte sind oft nicht nötig. Eine Hand auf die Schulter darf sein, wenn es sich stimmig anfühlt."
        },
        {
          eyebrow: "Stundenaufbau",
          titel: "Übergangslied nicht vergessen",
          body: "Nach dem Hauptteil das Tempo bewusst reduzieren: ruhigere Melodie, weniger Stimmvolumen. Das gibt der Gruppe Zeit, anzukommen, bevor das Abschlusslied beginnt."
        },
        {
          eyebrow: "Atemübung",
          titel: "Atemanlasser 4 – 4 – 8",
          body: "4 Sekunden einatmen, 4 Sekunden halten, 8 Sekunden ausatmen. Vagal aktivierend – beruhigt die Gruppe vor dem ersten Lied und hilft bei Unruhe."
        },
        {
          eyebrow: "Gruppenenergie",
          titel: "Bei Müdigkeit: Tempo raus",
          body: "Stimmungsabhängig reagieren: bei Unruhe → Rhythmus betonen; bei Traurigkeit → nicht forciert fröhlich sein; bei Müdigkeit → Tempo herausnehmen und Raum geben."
        }
      ];

      function dashTippDesTages() {
        var jetzt = new Date();
        var start = new Date(jetzt.getFullYear(), 0, 0);
        var diff = jetzt - start;
        var tagDesJahres = Math.floor(diff / (1000 * 60 * 60 * 24));
        return DASH_TIPPS[tagDesJahres % DASH_TIPPS.length];
      }

      function renderDashboard() {
        // ---- Favoriten-Zähler ----
        var favCount = (state.favoriten || []).length;
        var favEl = document.getElementById("dash-fav-count");
        if (favEl) favEl.textContent = favCount > 0 ? favCount + " Lieder" : "keine";

        // ---- Hero: Spickzettel oder letzte Setlist ----
        var heroArea = document.getElementById("dash-hero-area");
        if (!heroArea) return;

        var heroHtml = "";
        var spickAnzahl = (state.spickListe || []).length;
        ...
        if (spickAnzahl > 0) {
          ...
          // Letzte Setlist anzeigen
          var letzteSetlist = state.setlists[state.setlists.length - 1];
          var slAnzahl = (letzteSetlist.songs || []).length;
          heroHtml =
            '<div class="dash-hero">' +
              '<span class="dash-hero-eyebrow">Letzte Setlist</span>' +
              '<div class="dash-hero-title">' + escHtml(letzteSetlist.name || "Unbenannte Setlist") + '</div>' +
              '<div class="dash-hero-meta">' + slAnzahl + ' Lied' + (slAnzahl !== 1 ? 'er' : '') + '</div>' +
              '<button class="dash-hero-btn" onclick="startSetlistDirekt(' + letzteSetlist.id + ')">' +
                '&#9654; Direkt starten' +
              '</button>' +
            '</div>';
        } else {
          // Noch keine Setlist – leerer State
          var songAnzahl = (state.songs || []).length;
          heroHtml =
            '<div class="dash-hero">' +
              '<span class="dash-hero-eyebrow">Los geht\'s</span>' +
              '<div class="dash-hero-title">Bereit zum Singen?</div>' +
              '<div class="dash-hero-meta">' + songAnzahl + ' Lieder im Repertoire</div>' +
              '<button class="dash-hero-btn" onclick="showView(\'setlist\'); showSetlistSubtab(\'spick\');">' +
                SPICK_ICON + ' Spickzettel erstellen' +
              '</button>' +
            '</div>';
        }
        heroArea.innerHTML = heroHtml;

        // ---- Tipp des Tages ----
        var tipp = dashTippDesTages();
        var eyebrowEl = document.getElementById("dash-tip-eyebrow");
        var titleEl   = document.getElementById("dash-tip-title");
        var bodyEl    = document.getElementById("dash-tip-body");
        if (eyebrowEl) eyebrowEl.textContent = tipp.eyebrow;
        if (titleEl)   titleEl.textContent   = tipp.titel;
        if (bodyEl)    bodyEl.textContent    = tipp.body;
      }

      function dashZufallslied() {
        var songs = state.songs || [];
        if (!songs.length) {
          showToast("Noch keine Lieder im Repertoire.");
          return;
        }
        var zufaelligesLied = songs[Math.floor(Math.random() * songs.length)];
        showView("repertoire");
        // Kurz warten bis View gerendert, dann Lied öffnen
        setTimeout(function () {
          openSong(zufaelligesLied.id);
        }, 80);
      }

      // Sicher zum Handbuch-Tab "Stundenaufbau" navigieren.
      // Wird vom Dashboard-Tile aufgerufen; nutzt setTimeout damit
      // showView() den View erst aktiviert, bevor handbuchTab() die
      // Tab-Elemente sucht.
      function dashNavToHandbuchAufbau() {
        showView("handbuch");
        setTimeout(function () {
          var tabs = document.querySelectorAll(".handbuch-tab");
          var aufbauBtn = null;
          for (var i = 0; i < tabs.length; i++) {
            if (tabs[i].textContent.indexOf("Stundenaufbau") !== -1 ||
                tabs[i].getAttribute("onclick") && tabs[i].getAttribute("onclick").indexOf("aufbau") !== -1) {
              aufbauBtn = tabs[i];
              break;
            }
          }
          if (!aufbauBtn && tabs[2]) aufbauBtn = tabs[2];
          if (aufbauBtn) handbuchTab("aufbau", aufbauBtn);
        }, 60);
      }
      ...
      function befuelleFilterDropdowns() {
        ...
        // Tonart-Dropdown: unique Werte aus Song-Daten (kein fester Listenwert)
        var tonartSel = document.getElementById("filter-tonart");
        if (tonartSel) {
          var tonartCur = tonartSel.value;
          var tonartSeen = {};
          var tonarten = [];
          state.songs.forEach(function (s) {
            var t = (s.tonart || "").trim();
            if (t && !tonartSeen[t.toLowerCase()]) {
              tonartSeen[t.toLowerCase()] = true;
              tonarten.push(t);
            }
          });
          tonarten.sort(function (a, b) { return a.localeCompare(b, "de"); });
          tonartSel.innerHTML =
            '<option value="">Alle</option>' +
            tonarten
              .map(function (t) {
      ...
      function filterSongs() {
        ...
        var tonart =
          (document.getElementById("filter-tonart") || {}).value || "";
        var herkunft =
        ...
        var filtered = state.songs.filter(function (s) {
          ...
          if (tonart && (s.tonart || "").trim() !== tonart) return false;
          ...
          .map(function (song) {
            ...
            if (song.sprache)
              tags +=
                ...
                "</span>";
            // Origin-Badge (Zentral/Lokal) absichtlich weggelassen –
            // ist über die Filterfunktion (filter-herkunft) erreichbar.
        ...
        container.innerHTML = Object.keys(LISTEN_LABELS)
          .map(function (key) {
            ...
            return (
              ...
              '<summary><span class="settings-list-title">' +
      ...
      function renderCleanupSection(title, items, renderItem) {
        ...
        var html =
          ...
          '<summary class="cleanup-header"><span class="cleanup-header-title">' +
          escHtml(title) +
      ...
      function renderFavoriten() {
        ...
        container.innerHTML = favs
          .map(function (song) {
            var tags = "";
            if (song.tonart) tags += '<span class="tag tag-akkorde">' + escHtml(song.tonart) + "</span>";
            if (song.capo)   tags += '<span class="tag tag-capo">Capo ' + escHtml(song.capo) + "</span>";
            if (song.sprache) tags += '<span class="tag tag-akkorde">' + escHtml(song.sprache) + "</span>";
            var title = escHtml(song.titel);
            return (
              '<div class="song-card">' +
              '<button class="song-card-main" aria-label="Lied öffnen: ' +
      ...
      function renderSpickListe() {
        ...
        if (songs.length === 0) {
          container.innerHTML =
            '<div class="empty-state"><div class="icon" style="font-size:0;display:flex;justify-content:center;margin-bottom:var(--space-3)"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="44" height="44" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 3h9l4 4v14H6V3Z"/><path d="M14 3v6h5M9 13h6M9 17h5"/></svg></div><p>Noch leer.<br>Lieder aus dem Repertoire hinzuf\u00fcgen.</p></div>';
          return;
        }
        container.innerHTML = songs
          .map(function (song, i) {
      ...
      function renderSetlists() {
        ...
        // Header: immer sichtbar – Anzahl + Section-Toggle
        var header =
          '<div class="setlist-list-header" onclick="toggleAllSetlists()" role="button" tabindex="0" ' +
          'onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();toggleAllSetlists();}">' +
          '<span class="setlist-count-label">' + state.setlists.length +
          (state.setlists.length === 1 ? " Setlist" : " Setlisten") + "</span>" +
          '<span class="setlist-toggle-all-btn">' +
          (setlistSectionExpanded ? "&#9650; Alle einklappen" : "&#9660; Alle ausklappen") +
          "</span>" +
          "</div>";

        // Karten: nur wenn Section ausgeklappt
        var cardsHtml = "";
        if (setlistSectionExpanded) {
          cardsHtml = '<div class="setlists-cards-inner">' +
            state.setlists.map(function (sl) {
              var name = escHtml(sl.name);
              var used = sl.used_at
                ? "Zuletzt: " + new Date(sl.used_at).toLocaleDateString("de-DE")
                : "Noch nicht verwendet";
              var isExpanded = !!expandedSetlists[sl.id];
              var songItems = sl.songs.map(function (id, idx) {
                var song = state.songs.find(function (s) { return s.id === id; });
                if (!song) return "";
                var badge = song.tonart
                  ? '<span class="setlist-preview-badge">' + escHtml(song.tonart) +
                    (song.capo ? " · Capo " + escHtml(song.capo) : "") + "</span>"
                  : (song.capo
                    ? '<span class="setlist-preview-badge">Capo ' + escHtml(song.capo) + "</span>"
                    : "");
                return (
                  '<li class="setlist-preview-item">' +
                  '<span class="setlist-preview-num">' + (idx + 1) + ".</span>" +
                  '<span class="setlist-preview-title">' + escHtml(song.titel) + "</span>" +
                  badge + "</li>"
                );
              }).filter(Boolean).join("");
              return (
                '<div class="setlist-card' + (isExpanded ? " expanded" : "") +
                '" data-id="' + sl.id + '">' +
                '<div class="setlist-card-header" onclick="toggleSetlistExpand(' + sl.id + ')">' +
                '<span class="setlist-chevron">&#9658;</span>' +
                '<div class="setlist-card-info">' +
                '<div class="setlist-card-name">' + name + "</div>" +
                '<div class="setlist-card-meta">' + sl.songs.length + " Lieder | " + escHtml(used) + "</div>" +
                "</div>" +
                '<div class="setlist-card-actions">' +
                '<button class="card-btn" onclick="event.stopPropagation(); startSetlistDirekt(' + sl.id + ')" title="Setlist starten" aria-label="Setlist starten: ' + name + '">&#9654;</button>' +
                '<button class="card-btn" onclick="event.stopPropagation(); setlistInSpickzettelLadenDirekt(' + sl.id + ')" title="In Spickzettel laden" aria-label="Setlist in Spickzettel laden: ' + name + '">' + SPICK_ICON + '</button>' +
                '<button class="card-btn" onclick="event.stopPropagation(); duplicateSetlist(' + sl.id + ')" title="Duplizieren" aria-label="Setlist duplizieren: ' + name + '">&#10697;</button>' +
                '<button class="card-btn" onclick="event.stopPropagation(); openSetlistEditor(' + sl.id + ')" title="Bearbeiten" aria-label="Setlist bearbeiten: ' + name + '">&#9998;</button>' +
                "</div>" +
                "</div>" +
                '<div class="setlist-card-body">' +
                (sl.songs.length
                  ? '<ol class="setlist-preview-list">' + songItems + "</ol>"
                  : '<p style="margin:0;color:var(--color-muted);font-size:var(--text-meta);">Noch keine Lieder.</p>') +
                "</div>" +
                "</div>"
              );
            }).join("") +
      ...
      // Einzelne Karte auf-/zuklappen (zeigt/versteckt Liedliste)
      function toggleSetlistExpand(id) {
        expandedSetlists[id] = !expandedSetlists[id];
        var card = document.querySelector('.setlist-card[data-id="' + id + '"]');
        if (card) card.classList.toggle("expanded", !!expandedSetlists[id]);
      }
      window.toggleSetlistExpand = toggleSetlistExpand;

      // Gesamte Setlist-Section ein-/ausklappen
      function toggleAllSetlists() {
        setlistSectionExpanded = !setlistSectionExpanded;
        renderSetlists();
      }
      window.toggleAllSetlists = toggleAllSetlists;
      ...
      function renderLiedSuchOverlay() {
        ...
        document.getElementById("lied-such-liste").innerHTML = songs
          .map(function (song) {
            ...
            // Badges: Tonart, Capo, Sprache
            var badges = "";
            if (song.tonart) {
              badges += '<span class="tag tag-akkorde">' + escHtml(song.tonart) + '</span>';
            }
            if (song.capo) {
              badges += '<span class="tag tag-capo">Capo\u00a0' + escHtml(String(song.capo)) + '</span>';
            }
            if (song.sprache) {
              badges += '<span class="tag">' + escHtml(song.sprache) + '</span>';
            }
            var addIcon = inList ? "&#10003;" : "+";
            return (
              '<button class="picker-card' + (inList ? " in-list" : "") + '" onclick="liedSuchAuswaehlen(' + song.id + ')">' +
                '<span class="picker-title">' + escHtml(song.titel) + '</span>' +
                '<span class="picker-badges">' +
                  badges +
                  '<span class="picker-add">' + addIcon + '</span>' +
                '</span>' +
              '</button>'
            );
          })
      ...
      function handbuchTitelOhneEmoji(titel) {
        return String(titel || "")
      ...
      function renderHandbuch() {
        ...
        container.innerHTML = handbuchDaten
          .map(function (section, i) {
            var titleClean = handbuchTitelOhneEmoji(section.titel);
            return (
